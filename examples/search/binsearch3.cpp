...
// Binary Search
// Implementation #1: tail-recursive
...
// Throws what & when a ValueType operation (op<) throws.
// If no ValueType operation throws, then binSearch does not throw.
               ...
               const ValueType & findme)  // value to find
{
    while (true) {
        ...
        if (findme < *pivotIter) {
            // Recursively search in first half of range
            // return binSearch(first, pivotIter, findme);
            last = pivotIter;
        } else {
            // Recursively search in second half of range
